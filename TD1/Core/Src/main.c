...
/* USER CODE BEGIN PV */
	...
	int state = 1;
	uint16_t PinList[4] = {Green_Pin,Orange_Pin,Red_Pin,Blue_Pin};
/* USER CODE END PV */
...
int i = 1;
  while (1)
  {
	  ...
	  */
	   ...
	   * 6.1.3
	   *
	   * Pbutton_State = HAL_GPIO_ReadPin(GPIOA,Button_Pin);
	   *
	   * */
	  for(int i =0; i<4; i++){
			  HAL_GPIO_TogglePin(GPIOD, PinList[i]);
			  if (HAL_GPIO_ReadPin (GPIOD,PinList[i>0 ? i-1 : 3])){
				  HAL_GPIO_TogglePin(GPIOD, PinList[i>0 ? i-1 : 3]);
			  }
			  HAL_Delay((HAL_GPIO_ReadPin(GPIOA,Button_Pin) ? 200 : 1000));
		  }
